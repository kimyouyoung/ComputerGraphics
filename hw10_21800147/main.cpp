...
// define each models(with appropriate parameters)
CubePrimitive cube(1.0f, 1.0f, 1.0f);
SpherePrimitive sphere(0.5f, 30, 30);
ConePrimitive cone(0.5f, 1.0f, 15);
...
// set color(different with vColor)
void set_color(GLfloat r, GLfloat g, GLfloat b, GLfloat a = 1.0f)
{
	using namespace glm;
	// mode(location=2): 3 -> fColor = uColor(specified Color -> vec4(r, g, b, a));
	glUniform1i(2, 3);
	// uColor(location=3): vec4(r, g, b, a)
...
void init()
{
	...
	// add each of models in models(vector)
...
void display()
{
	...
	// mode -> 0: original color, 1: phong shading, 2: gouraud shading 
	glUniform1i(2, mode);

	GLint location;

	// transformation matrix --> additional model
	mat4 T(1.0f);
	...
	if (show_wireframe)
	{
		...
		set_color(0, 0, 0);
		glLineWidth(1.0f);
		// mode(location=2): 3 -> fColor = uColor
		glUniform1i(2, 3);
...
void specialkey(int key, int x, int y)
{
	switch (key)
	{
	// turn left: tires(left and right) rotated opposite direction
	case GLUT_KEY_HOME:
	...
	// turn right: tires(left and right) rotated opposite direction
	case GLUT_KEY_END:
		model_state.theta -= 0.005f;
		car.turn_right();
		glutPostRedisplay();
	...
	// move forward: same direction to move forward
	case GLUT_KEY_LEFT:
		using namespace glm;
	...
	// move backward: same direction to move backward
	case GLUT_KEY_RIGHT:
		using namespace glm;
