...
public:
	...
	virtual void init(GLint program)
	{
		...
		// vNormal --> normal vectors
		const GLchar* attri_name[3] = { "vPosition", "vNormal", "vColor" };
...
struct CubePrimitive : public Model
{
	...
	{
		get_cube_3d(vtx_pos, vtx_nml, lx, ly, lz);
...
struct SpherePrimitive : public Model
{
	...
	{
		get_sphere_3d(vtx_pos, vtx_nml, radius, lats, longs);
		get_color_3d_by_pos(vtx_clrs, vtx_pos);
	}
...
struct ConePrimitive : public Model
{
	...
	{
		get_cone_3d(vtx_pos, vtx_nml, idx_list[0], idx_list[1], radius, height, subdivs);
		get_color_3d_by_pos(vtx_clrs, vtx_pos);
	}
...
struct CylinderPrimitive : public Model
{
	...
	{
		get_cylinder_3d(vtx_pos, vtx_nml, idx_list[0], idx_list[1], idx_list[2], radius, height, subdivs);
		get_color_3d_by_pos(vtx_clrs, vtx_pos);
	}
...
struct TorusPrimitive : public Model
{
	...
	{
		get_torus_3d(vtx_pos, vtx_nml, idx_lists, radius0, radius1, longs, lats);
		get_color_3d_by_pos(vtx_clrs, vtx_pos);
	}
...
struct CarModel : public Model
{
	...
	// turn left: left -> "-", right -> "+" : opposite direction
	void turn_left()
	{
	...
	// turn right: left -> "+", right -> "-" : opposite direction
	void turn_right()
	{
		wheel_angle_L += 0.015f;
		wheel_angle_R -= 0.015f;
