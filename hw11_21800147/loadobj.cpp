	...
	tinyobj::real_t scale)
{
	...
	// Load the obj file.
	bool ret = LoadObj(&attrib, &shapes, &materials, &warn,
	...
	for (size_t s = 0; s < num_of_shapes; ++s) {
		...
		if (faces.size() > 0)
		{
			size_t current_id = mesh.material_ids[0];
			vertex_map[s].push_back(num_of_out_vertices);
			material_map[s].push_back(current_id);

			for (size_t f = 0; f < faces.size(); ++f)
			{
				size_t material_id = mesh.material_ids[f];
				if (current_id != material_id) {
					vertex_map[s].push_back(num_of_out_vertices);
					material_map[s].push_back(material_id);
